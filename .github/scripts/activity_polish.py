...
p = Path('netlify/functions/_ops-line-ui.ts')
s = p.read_text()

old = """  } else if (input.status === 'confirmed') {
...
"""
new = """  } else if (input.status === 'confirmed') {
    rows.push(actionButton('✅ เสร็จงาน', {
      type: 'postback',
...
marker = "function cancelConfirmMessage(booking: BookingUiRow, serviceName: string): LineMessage {\n"
insert = """function completeConfirmMessage(booking: BookingUiRow, serviceName: string): LineMessage {
  return {
    type: 'template',
...
old2 = """  if (action === 'cancel_prompt') return [cancelConfirmMessage(booking, info.name)];
  if (action === 'cancel_no') return [textMessage('โอเคครับ งานยังอยู่เหมือนเดิม')];

  try {
"""
new2 = """  if (action === 'complete' || action === 'complete_prompt') {
...
"""
if old2 not in s:
    raise SystemExit('pre-action marker not found')
s = s.replace(old2, new2, 1)

old3 = """    if (action === 'complete') {
...
"""
if old3 not in s:
    raise SystemExit('complete marker not found')
s = s.replace(old3, new3, 1)

old4 = "      return [simpleSuccess('ยกเลิกงานแล้ว', 'คืนคิวและแจ้งลูกค้าแล้ว')];\n"
new4 = """      const fresh = await bookingByCode(code);
      return [
...
"""
if old4 not in s:
    raise SystemExit('cancel response marker not found')
