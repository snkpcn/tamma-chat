...
test('semantic-v27 preserves the three semantic-v24 live-failure gold meanings',()=>{
...
test('semantic-v27 makes explicit transaction commitment outrank catalog browsing',()=>{
...
test('semantic-v27 makes constraint-only continuation provide information',()=>{
  const p=buildSemanticInterpreterPrompt(emptySemanticContext()).replace(/\s+/g,' ');
...
test('semantic-v27 requires an evaluative request beyond companion metadata',()=>{
  const p=buildSemanticInterpreterPrompt(emptySemanticContext()).replace(/\s+/g,' ');
