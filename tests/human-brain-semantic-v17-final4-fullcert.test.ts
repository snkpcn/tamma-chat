...
test('semantic-v19 keeps all four final full-corpus gold labels unchanged',()=>{
...
test('semantic-v19 promotion ownership outranks the promoted business category',()=>{
...
test('semantic-v19 distinguishes one explicit activity target from a generic multi-step flow',()=>{
  const p=buildSemanticInterpreterPrompt(emptySemanticContext()).replace(/\s+/g,' ');
...
test('semantic-v19 payment remediation keeps payment domain rather than generic support',()=>{
  const p=buildSemanticInterpreterPrompt(emptySemanticContext()).replace(/\s+/g,' ');
...
test('semantic-v19 structurally makes recommend imply recommendation facet when model leaves it none',()=>{
