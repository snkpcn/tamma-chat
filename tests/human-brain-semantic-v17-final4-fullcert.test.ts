...
test('semantic-v24 keeps all four final full-corpus gold labels unchanged',()=>{
...
test('semantic-v24 promotion ownership outranks the promoted business category',()=>{
...
test('semantic-v24 distinguishes one explicit activity target from a generic multi-step flow',()=>{
  const p=buildSemanticInterpreterPrompt(emptySemanticContext()).replace(/\s+/g,' ');
...
test('semantic-v24 payment remediation keeps payment domain rather than generic support',()=>{
  const p=buildSemanticInterpreterPrompt(emptySemanticContext()).replace(/\s+/g,' ');
...
test('semantic-v24 structurally makes recommend imply recommendation facet when model leaves it none',()=>{
