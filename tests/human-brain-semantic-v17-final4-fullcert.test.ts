...
test('semantic-v28 promotion ownership outranks the promoted business category',()=>{
...
test('semantic-v28 distinguishes one explicit activity target from a generic multi-step flow',()=>{
  const p=buildSemanticInterpreterPrompt(emptySemanticContext()).replace(/\s+/g,' ');
...
test('semantic-v28 payment remediation keeps payment domain rather than generic support',()=>{
  const p=buildSemanticInterpreterPrompt(emptySemanticContext()).replace(/\s+/g,' ');
...
test('semantic-v28 structurally makes recommend imply recommendation facet when model leaves it none',()=>{
