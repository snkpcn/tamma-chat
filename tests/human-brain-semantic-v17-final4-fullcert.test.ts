...
test('semantic-v21 keeps all four final full-corpus gold labels unchanged',()=>{
...
test('semantic-v21 promotion ownership outranks the promoted business category',()=>{
...
test('semantic-v21 distinguishes one explicit activity target from a generic multi-step flow',()=>{
  const p=buildSemanticInterpreterPrompt(emptySemanticContext()).replace(/\s+/g,' ');
...
test('semantic-v21 payment remediation keeps payment domain rather than generic support',()=>{
  const p=buildSemanticInterpreterPrompt(emptySemanticContext()).replace(/\s+/g,' ');
...
test('semantic-v21 structurally makes recommend imply recommendation facet when model leaves it none',()=>{
