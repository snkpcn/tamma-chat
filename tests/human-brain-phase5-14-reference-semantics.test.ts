...
test('semantic-v17 locks short topic follow-up vs invented availability',()=>{
...
test('semantic-v17 locks exact named selection among multiple recent candidates',()=>{
  const prompt=buildSemanticInterpreterPrompt(emptySemanticContext());
...
test('semantic-v17 distinguishes candidate-slot question from slot supply',()=>{
  const prompt=buildSemanticInterpreterPrompt(emptySemanticContext());
...
test('semantic-v17 explicit attribute comparison outranks recommendation',()=>{
  const prompt=buildSemanticInterpreterPrompt(emptySemanticContext());
