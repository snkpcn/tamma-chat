...
test('semantic-v24 keeps generic action verbs inside broad ecosystem scope',()=>{
  ...
  assert.ok(p.includes('Generic action-like experience verbs alone do not narrow ecosystem scope to activity'));
...
test('semantic-v24 treats companion context as constraints, not recommendation intent',()=>{
  const p=buildSemanticInterpreterPrompt(emptySemanticContext()).replace(/\s+/g,' ');
  assert.ok(p.includes('Traveler, companion, or group context by itself is a constraint, not a request for curation'));
  assert.ok(p.includes('A neutral what-is-there-to-do request remains ecosystem + discover even when the customer says they came with a partner, family, children, elders, or friends'));
