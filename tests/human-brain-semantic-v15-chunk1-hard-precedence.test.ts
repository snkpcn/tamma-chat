...
test('semantic-v16 version is explicit',()=>{
  assert.equal(SEMANTIC_INTERPRETER_VERSION,'semantic-v16');
});

test('semantic-v16 honestly adjudicates membership profile readback as ask',()=>{
...
test('semantic-v16 final precedence protects how-it-works questions from catalog discovery',()=>{
...
test('semantic-v16 final precedence protects selected-option slot refinement from transaction escalation',()=>{
  const normalized=buildSemanticInterpreterPrompt(emptySemanticContext()).replace(/\s+/g,' ');
...
test('semantic-v16 final precedence makes explicit canonical category ownership override venue framing',()=>{
  const normalized=buildSemanticInterpreterPrompt(emptySemanticContext()).replace(/\s+/g,' ');
