...
test('semantic-v19 honestly adjudicates membership profile readback as ask',()=>{
...
test('semantic-v19 final precedence protects how-it-works questions from catalog discovery',()=>{
...
test('semantic-v19 final precedence protects selected-option slot refinement from transaction escalation',()=>{
  const normalized=buildSemanticInterpreterPrompt(emptySemanticContext()).replace(/\s+/g,' ');
...
test('semantic-v19 final precedence makes explicit canonical category ownership override venue framing',()=>{
  const normalized=buildSemanticInterpreterPrompt(emptySemanticContext()).replace(/\s+/g,' ');
