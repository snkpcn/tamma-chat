...
test('semantic-v15 keeps journey-02 and informational-02 gold unchanged',()=>{
...
test('semantic-v15 removes the activity-domain contradiction and gives explicit category nouns priority',()=>{
...
test('semantic-v15 distinguishes retrieving an existing saved artifact from catalog discovery',()=>{
  const prompt=buildSemanticInterpreterPrompt(emptySemanticContext());
  const normalized=prompt.replace(/\s+/g,' ');
