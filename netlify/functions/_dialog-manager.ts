...
function planKnowledgeNeeds(turn: SemanticTurn, container: TaskStateContainer): KnowledgeRequest[] {
  ...
  switch (turn.domain) {
    case 'restaurant':
      // Human Brain Phase 1.3: route from the CLOSED semantic information
      // facet, never from an arbitrary free-form intent label the model may
      // phrase differently across equally-correct turns.
      if (turn.informationNeed === 'availability') {
        return [{ ...base, domain: 'restaurant', needs: ['availability'] }];
      }
      if (turn.informationNeed === 'price') {
        return [{ ...base, domain: 'restaurant', needs: ['price'] }];
      }
      if (turn.informationNeed === 'ingredients') {
        return [{ ...base, domain: 'restaurant', needs: ['ingredients'] }];
      }
      if (turn.informationNeed === 'transaction_status') {
        return [{ ...base, domain: 'restaurant', needs: ['order_status'] }];
      }
      // Compatibility for mature deterministic turns which do not yet carry
      // the optional facet. Status retains its old order-status meaning only
      // when no explicit machine-facing information need was supplied.
