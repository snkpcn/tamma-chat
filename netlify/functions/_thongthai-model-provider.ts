...
// A 429 from one model is therefore not proof that every other free Gemini
// model is unavailable, so runtime keeps one bounded circuit per model and may
// still try another free model inside the same latency budget. Conversely,
// certification must pace requests project-wide because a shared/project quota
// can legitimately rate-limit several model IDs at once.
