...
const DISCOVERY_PATTERNS = [
  ...
  /แถวนี้(?:มี)?(?:อะไร|ไร)(?:ให้)?ทำ(?:ดี|บ้าง)?/u,
  /พาแฟนมา.*(?:มีอะไร|มีไร|แนะนำ|ทำอะไร|ทำไร)/u,
  /(?:มาครั้งแรก|ครั้งแรก).*(?:มีอะไรแนะนำ|แนะนำอะไร)/u,
  /(?:มาครั้งแรก|ครั้งแรก).*แนะนำ/u,
];
...
function normalizeThaiDiscoveryText(message: string): string {
  ...
  text = text
    ...
    .replace(/ทำ(?:รัย|ไร)(?=\s|ดี|แนะนำ|$)/gu, 'ทำอะไร')
...
export function isExperienceDiscoveryIntent(message: string): boolean {
  ...
  const hasDiscoveryShape =
    ...
    || /(?:แถวนี้).*(?:อะไร|ไร).*(?:ทำ|เล่น|เที่ยว|ดี)/u.test(text)
    || /(?:พาแฟนมา).*(?:แนะนำ|ทำ|เที่ยว|เล่น|อะไร|ไร)/u.test(text)
