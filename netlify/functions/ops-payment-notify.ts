...
import { dispatchPaymentNotification } from './_payments';
...
export const handler: Handler = async (event: HandlerEvent) => {
  ...
  try {
    const status = await dispatchPaymentNotification(body.id);
    return {
      statusCode: 200,
    ...
    console.error('PAYMENT_NOTIFICATION_ERROR', error instanceof Error ? error.message.slice(0, 320) : 'unknown');
    return { statusCode: 500, body: 'Payment notification failed' };
  }
};
